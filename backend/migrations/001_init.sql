...
-- Which service route a day's route was prepared for. Null on one-offs.
...
--
-- Rows are disposable: they are deleted the moment a code is used, and
-- any that outlive their expiry are swept on the next request for the
-- same number. No background job — an abandoned challenge is a few bytes,
-- and a cron to tidy them would be more moving parts than the problem
-- deserves.
